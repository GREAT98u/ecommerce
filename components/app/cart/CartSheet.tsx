...
export function CartSheet() {
  ...
  // derive total items locally (cheap + safe)
  const totalItems = items.reduce(
    (sum, item) => sum + item.quantity,
    0
  );

  const { stockMap, isLoading, hasStockIssues } =
    useCartStock(items);
        ...
        {items.length === 0 ? (
          ...
          <>
            ...
            {hasStockIssues && !isLoading && (
              <div className="mx-5 mt-2 flex items-center gap-2 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-sm text-amber-800">
            ...
            <div className="flex-1 overflow-y-auto px-5">
              <div className="divide-y py-2 space-y-2">
                {items.map((item) => (
                  <CartItem
                    key={item.productId}
                    item={item}
